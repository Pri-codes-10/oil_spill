...
def predict(tile):
    ...
    for lab in range(1, n + 1):
        ...
        if area < THRESHOLD_MIN_PIXELS:              # speckle, not a slick
            continue
        # threshold_local() returns the THRESHOLD (local_mean - offset), not
        # the local mean, so this difference understates true dark-contrast by
        # exactly THRESHOLD_OFFSET dB. Add it back before scaling, or every
        # scene's confidence comes out low and Contract 1 under-reports.
        contrast = float(
            local[mask].mean() - vv[mask].mean() + THRESHOLD_OFFSET
        )
